...
- (void)drawRect:(CGRect)rect{ 
    NSArray *arr = [self.subviews copy];
...
- (void)drawX{
    ...
    if (!_xqdColor.isMoreLine) {
        CGFloat y1 = (x_h - (x_h/_xqdColor.chartLine_YMax)*[GL_USERDEFAULTS getFloatValue:SamTargetHeight]);
        CGFloat y2 = (x_h - (x_h/_xqdColor.chartLine_YMax)*[GL_USERDEFAULTS getFloatValue:SamTargetLow]);
        UIBezierPath *path = [UIBezierPath bezierPathWithRect:CGRectMake(CHARTLINE_Y_LEFT, y1, x_w-CHARTLINE_Y_LEFT, y2-y1)];
        path.lineWidth     = 1.0;
        path.lineCapStyle  = kCGLineCapRound;
        path.lineJoinStyle = kCGLineJoinBevel;
        UIColor *fillColor = TCOL_HISTORYCELL;
        [fillColor set];
        [path fill];
        [path stroke];
    }
...
//        layer_x.path = path_x.CGPath;
    ...
    for (int i=0; i<_xqdColor.chartLine_Xarr.count; i++) {
        ...
        if (i%((int)cout*2)==0) {//每个x轴点的间隔数量
            ...
            strX -= [tmpWidthLbl getLabelWidth] / 2;
            ...
            UIFont *font = self.width < SCREEN_WIDTH - _xqdColor.chartLine_leftDistance ? GL_FONT(11) : GL_FONT(CHARTLINE_Y_FONT);
            
            [str drawInRect:CGRectMake(strX, x_h + 2, [tmpWidthLbl getLabelWidth], 20) withAttributes:@{NSFontAttributeName:font,NSForegroundColorAttributeName:_xqdColor.chartLine_XYTextColor}];
        }
    }
}
...
//        layer_line.path = path_line.CGPath;
    ...
    if (!_xqdColor.isMoreLine) {
        if ([_delegate respondsToSelector:@selector(reloadMainSVWithPointX:)]) {
            [_delegate reloadMainSVWithPointX:tmpX];
        }
    }
}
...
//        [image drawInRect:CGRectMake(x, y, 10, 16)];//在坐标中画出图片
        ...
        GLButton *btn =[[GLButton alloc]initWithFrame:CGRectMake(x - 10, y - 8, 10 * 2, 16 * 2)];
